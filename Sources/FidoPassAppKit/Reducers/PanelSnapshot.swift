...
/// stores. The HUD's click budget — one Return from an unlocked key to a copied password —
/// is enforced here rather than described: with a key unlocked and an account preselected
/// the answer must be `generateAndCopy`, not "select an account first".
