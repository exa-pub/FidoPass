...
/// from versions that predate identities, 60 from this one. Both have to parse, and the
...
final class PortableBackupTests: XCTestCase {
    ...
    private let identity = AccountIdentity(bytes: Data((0..<12).map { UInt8($0 &* 3) }))!
    ...
    func testCurrentBackupRoundTrips() throws {
        ...
        XCTAssertEqual(backup.base64.count, 60)
        XCTAssertEqual(Data(base64Encoded: backup.base64)?.count, 44)
    ...
    /// The 44-character form is what every earlier version printed. It parses, and it says
    /// what it is.
