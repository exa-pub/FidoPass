...
enum MessageFixtures {
    ...
    static func url(nonce: Data = nonce, scalar: Data = scalar) throws -> EncryptionKeyURL {
        try EncryptionKeyURL(nonce: nonce,
                             publicKey: try MessageKey.publicKey(for: scalar),
                             locator: try locator(nonce: nonce))
    }

    static func key(nonce: Data = nonce, scalar: Data = scalar) throws -> MessageKey {
        MessageKey(url: try url(nonce: nonce, scalar: scalar), scalar: scalar)
    }
}
